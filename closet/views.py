...
def createLook(request):
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse('looks'))
...
def look(request, id):
    look = Look.objects.get(id=id)
    clothes = look.clothes.all()
    
    return render(request, 'closet/look.html', {
        'look': look,
        'clothes': clothes
    })

def deleteLook(request, id):
    look = Look.objects.get(id=id)
    if (look.user == request.user):
        look.delete()

    return HttpResponseRedirect(reverse('looks'))


def editLook(request, id):
    if request.method == "POST":
        name = request.POST["name"]
        try:
            image = request.FILES['image']
        ...
        editedLook = Look.objects.get(id=id)
        editedLook.name = name
        editedLook.image = image
        editedLook.save()

        editedLook.clothes.clear()
        editedLook.save()
        ...
        while True:
            try:
                clothIndex = request.POST[("clothingItem_id" + str(i))]
                editedLook.clothes.add(clothIndex)
                editedLook.save()
            except:
                break
            i += 1
    ...
    return render(request, 'closet/editLook.html', {
        'clothes': ClothingItem.objects.filter(user=request.user),
        'look': Look.objects.get(id=id),
        'lookClothes': Look.objects.get(id=id).clothes.all()
    })
